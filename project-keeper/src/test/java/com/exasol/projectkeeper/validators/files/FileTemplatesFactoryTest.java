...
class FileTemplatesFactoryTest {
    ...
    private FileTemplatesFactory testee(final boolean hasNpmModule, final List<AnalyzedSource> sources) {
        return testee(hasNpmModule, sources, BuildOptions.builder().runnerOs("ci-build-runner-os").build());
    }
    ...
    void testProjectKeeperVerifyYml(final boolean hasNpmModule) {
        ...
        assertThat(content, allOf(not(containsString("$npmSetup")), //
                ...
                containsString("uses: actions/setup-node@v6\n" +
                        "        if: ${{ " + hasNpmModule + " }}")));
    }

    @Test
    void acceptsProjectKeeperVerifyWorkflowCustomizationForNonMavenProject() {
        final List<AnalyzedSource> sources = List.of(AnalyzedMavenSource.builder()
                .modules(Collections.emptySet()).isRootProject(false).build());
        final BuildOptions options = BuildOptions.builder()
                .workflows(List.of(CustomWorkflow.builder().workflowName("project-keeper-verify.yml").build()))
                .build();
        assertDoesNotThrow(() -> testee(false, sources, options).getGlobalTemplates());
    }

    @Test
    void failsForUnusedWorkflowCustomization() {
        final List<AnalyzedSource> sources = getMavenSourceWithModules(Set.of());
        final BuildOptions options = BuildOptions.builder()
                .workflows(List.of(CustomWorkflow.builder().workflowName("does-not-exist.yml").build()))
                .build();
        final FileTemplatesFactory testee = testee(false, sources, options);
        final IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, testee::getGlobalTemplates);
        assertThat(exception.getMessage(), equalTo(
                "E-PK-CORE-198: Workflow customizations ['does-not-exist.yml'] in file '.project-keeper.yml' are not used. "
                        + "Remove the customizations or configure workflows generated by Project Keeper."));
    }

    @Test
    void failsForProjectKeeperVerifyWorkflowCustomizationForMavenProject() {
        final List<AnalyzedSource> sources = getMavenSourceWithModules(Set.of());
        final BuildOptions options = BuildOptions.builder()
                .workflows(List.of(CustomWorkflow.builder().workflowName("project-keeper-verify.yml").build()))
                .build();
        ...
        final IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, testee::getGlobalTemplates);
        assertThat(exception.getMessage(), equalTo(
                "E-PK-CORE-198: Workflow customizations ['project-keeper-verify.yml'] in file '.project-keeper.yml' are not used. "
                        + "Remove the customizations or configure workflows generated by Project Keeper."));
    }
