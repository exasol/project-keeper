...
class ProjectVersionDetectorTest {
    ...
    void testSpecifiedSourceNotFound() {
        ...
        final IllegalArgumentException exception = assertThrows(IllegalArgumentException.class,
                () -> versionDetector.detectVersion(config, sources));
        assertThat(exception.getMessage(), equalTo(
                "E-PK-CORE-114: Failed to detect overall project version. Could not find a source with specified path 'pom.xml'. Please make sure that you defined a source with exactly the same path."));
    }
