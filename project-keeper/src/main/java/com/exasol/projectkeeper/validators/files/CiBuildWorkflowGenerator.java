...
class CiBuildWorkflowGenerator {
    ...
    private final Set<String> usedWorkflowNames = new HashSet<>();
    ...
    private FileTemplate createTemplate(final FileTemplateFromResource template, final String workflowName,
        ...
        final Optional<CustomWorkflow> workflow = findWorkflow(workflowName);
    ...
    private List<StepCustomization> findCustomizations(final String workflowName) {
        return findWorkflow(workflowName)
    ...
    void verifyAllWorkflowCustomizationsAreUsed() {
        final List<String> unusedWorkflowNames = buildOptions.getWorkflows().stream()
                .map(CustomWorkflow::getWorkflowName)
                .filter(workflowName -> !usedWorkflowNames.contains(workflowName))
                .toList();
        if (!unusedWorkflowNames.isEmpty()) {
            throw new IllegalArgumentException(ExaError.messageBuilder("E-PK-CORE-198")
                    .message("Workflow customizations {{workflow names}} in file {{config file name}} are not used.",
                            unusedWorkflowNames, ProjectKeeperConfigReader.CONFIG_FILE_NAME)
                    .mitigation("Remove the customizations or configure workflows generated by Project Keeper.")
                    .toString());
        }
    }
    ...
    private FileTemplate createCustomizedWorkflow(final String templateResource, final String workflowName,
        ...
        final List<String> jobstoRemove = findWorkflow(workflowName)
                .map(CustomWorkflow::getRemovedJobs).orElseGet(Collections::emptyList);
