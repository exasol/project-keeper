...
public class GitHubAdapter {
    ...
    GitHubAdapter(final Github gitHub, final String owner, final String repoName) {
        this.gitHub = gitHub;
        this.owner = owner;
    ...
    public static GitHubAdapter connect(final String repoName) {
        ...
        return new GitHubAdapter(adapter, "exasol", repoName);
    }
    ...
    public Set<Integer> getClosedIssues() {
        ...
        final Set<Integer> issues = StreamSupport
                .stream(gitHub.repos().get(new Coordinates.Simple(owner, repoName)).issues()
                        .search(Sort.CREATED, Order.ASC, qualifiers).spliterator(), false)
                .map(Issue::number).collect(toSet());
