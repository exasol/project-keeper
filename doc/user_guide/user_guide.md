...
In order to send notifications, this workflow requires GitHub secrets `INTEGRATION_TEAM_SLACK_NOTIFICATION_WEBHOOK` (to report failure status for dependency check) and `INTEGRATION_TEAM_SLACK_NOTIFICATION_WEBHOOK` (to report new pull request with dependency update).
