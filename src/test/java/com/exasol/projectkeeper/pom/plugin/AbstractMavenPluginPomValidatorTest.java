...
public abstract class AbstractMavenPluginPomValidatorTest {
    ...
    private void runAllFixesGeneratedByValidation(final Document pom,
        ...
        this.template.validate(pom, Collections.emptyList(), finding -> finding.getFix().fixError());
    }
    ...
    protected void runFixesFromValidatePluginConfiguration(final Node plugin,
            final Collection<ProjectKeeperModule> enabledModules) {
        this.template.validatePluginConfiguration(plugin, enabledModules, finding -> finding.getFix().fixError());
    }
}
